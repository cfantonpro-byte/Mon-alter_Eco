...
(function(){
  ...
  const PREMIUM_ICON={
    ...
    roundProtection:'Bouton_bouclier.png',
    roundGrowth:'Bouton_graph.png',
    roundSavings:'Bouton_tirelire.png',
    ...
    birth:'Icone_SiteV2/Calendrier.png',
    tax:'assets/icone profil/Impots.png',
    capital:'Icone_SiteV2/Coffre_fort.png',
    monthly:'assets/result-semantic-v1/capacity-wallet.png',
    outlook:'Icone_SiteV2/telescope.png',
    progress:'Icone_SiteV2/classement.png',
    routes:'Icone_SiteV2/direction.png',
    portfolio:'Icone_SiteV2/Dossier.png',
    review:'Icone_SiteV2/Fichier_Loupe.png',
  ...
  function renderSynthese(){
    ...
    return `<section class="tabview" data-view="synthese">
      ...
      <div class="tv-col-main" style="margin-top:22px">
        ...
        <div class="plan-stepper">${planSteps.map(([label,done,detail],i)=>`<details class="plan-step ${done?'is-done':'is-current'}"><summary><span class="plan-step-dot"><i>${done?AppShell.icon('check',14):i+1}</i></span><span><b>${i+1}. ${esc(label)}</b><small>${done?'Terminé':'À venir'}</small></span><i class="plan-step-toggle" aria-hidden="true"><img class="step-expand" src="Icone_SiteV2/Plus.png" alt=""><img class="step-collapse" src="Icone_SiteV2/Moins.png" alt=""></i></summary><div class="plan-step-detail"><p>${detail}</p></div></details>`).join('')}</div>
      </div>
  ...
  function paint(){
    ...
    if(resultAvatar){
      resultAvatar.src=tab==='potentiel'?'Genie_loupe.PNG':tab==='synthese'?'Icone_Genie/genie_valide.png':'assets/walter-result-avatar.png';
  ...
  function showCityPhoto(){
    ...
    document.getElementById('drawer-illustration').src=encodeURI(`photo studyz/photo new/${file}.png`);
